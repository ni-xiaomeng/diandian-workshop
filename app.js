...
const convertGuideOn = document.getElementById("convertGuideOn");
const convertGuideInterval = document.getElementById("convertGuideInterval");
const convertGuideColor = document.getElementById("convertGuideColor");
const drawGuideOn = document.getElementById("drawGuideOn");
const drawGuideInterval = document.getElementById("drawGuideInterval");
const drawGuideColor = document.getElementById("drawGuideColor");
...
function drawGuideLines(ctx, cols, rows, pixelSize, interval, color) {
  const w = cols * pixelSize;
  const h = rows * pixelSize;
  ctx.save();
  ...
  for (let y = interval; y < rows; y += interval) {
...
function drawDrawCanvas() {
  ...
  if (drawGuideOn && drawGuideOn.checked) {
    const interval = Math.max(2, Math.min(20, Number(drawGuideInterval.value) || 5));
    drawGuideLines(drawCtx, drawGridCols, drawGridRows, drawPixelSize, interval, drawGuideColor.value);
  }
...
function drawConvertCanvas() {
  ...
  if (convertGuideOn && convertGuideOn.checked) {
    const interval = Math.max(2, Math.min(20, Number(convertGuideInterval.value) || 5));
    drawGuideLines(convertCtx, convertGridCols, convertGridRows, convertPixelSize, interval, convertGuideColor.value);
  }
  syncConvertOverlayToCanvas();
...
function refreshColorStats(kind) {
  ...
  for (const [hex, n] of show) {
    ...
    label.className = "color-swatch-count";
...
[convertGuideOn, convertGuideInterval, convertGuideColor].forEach(el => {
  if (el) el.addEventListener("change", () => drawConvertCanvas());
  if (el) el.addEventListener("input", () => drawConvertCanvas());
});
[drawGuideOn, drawGuideInterval, drawGuideColor].forEach(el => {
  if (el) el.addEventListener("change", () => drawDrawCanvas());
  if (el) el.addEventListener("input", () => drawDrawCanvas());
});

(function wireGuideColorPreviews() {
  const sync = (input, preview) => {
    if (!input || !preview) return;
    preview.style.backgroundColor = input.value;
  };
  const dIn = drawGuideColor;
  const dPr = document.getElementById("drawGuideColorPreview");
  const cIn = convertGuideColor;
  const cPr = document.getElementById("convertGuideColorPreview");
  sync(dIn, dPr);
  sync(cIn, cPr);
  if (dIn && dPr) {
    dIn.addEventListener("input", () => sync(dIn, dPr));
    dIn.addEventListener("change", () => sync(dIn, dPr));
  }
  if (cIn && cPr) {
    cIn.addEventListener("input", () => sync(cIn, cPr));
    cIn.addEventListener("change", () => sync(cIn, cPr));
  }
})();
